...
RSpec.describe "bundler plugin install" do
  ...
  context "Gemfile eval" do
    ...
    context "with plugins_in_lockfile" do
      ...
      it "does not include plugins as dependencies for an existing lockfile", bundler: "< 3" do
        install_gemfile <<-G
          source 'https://gem.repo2'
          gem 'myrack', "1.0.0"
        G
        ...
        bundle "config set --local plugins_in_lockfile true"
        bundle "install"

        expect(the_bundle).to include_gems("foo 1.0.0")

        # but will not remove them, once they're there, regardless of the setting
        bundle "config set --local plugins_in_lockfile false"
        bundle "install"

        expect(the_bundle).to include_gems("foo 1.0.0")
      end

      it "includes plugins as dependencies for an existing lockfile", bundler: "3" do
        install_gemfile <<-G
          source 'https://gem.repo2'
          gem 'myrack', "1.0.0"
        G
