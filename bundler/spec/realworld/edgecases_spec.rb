...
RSpec.describe "real world edgecases", :realworld => true do
  ...
  it "outputs a helpful error message when gems have invalid gemspecs", :rubygems => "< 3.3.14" do
  ...
  it "outputs a helpful warning when gems have a gemspec with invalid `require_paths`", :rubygems => ">= 3.3.14" do
    install_gemfile <<-G, :standalone => true, :env => { "BUNDLE_FORCE_RUBY_PLATFORM" => "1" }
      ...
      gem "ruby2_keywords", "0.0.5"
    G
    expect(err).to include("resque-scheduler 2.2.0 includes a gemspec with `require_paths` set to an array of arrays. Newer versions of this gem might've already fixed this")
  end
