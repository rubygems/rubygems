...
RSpec.describe "fetching dependencies with a not available mirror", :realworld => true do
  ...
  context "with a specific mirror without a fallback timeout" do
    ...
    it "fails to install the gem with a timeout error" do
      ...
      expect(err).to include("Retrying fetcher due to error (2/4): Bundler::HTTPError Could not fetch specs from #{mirror}/ due to underlying error <")
      expect(err).to include("Retrying fetcher due to error (3/4): Bundler::HTTPError Could not fetch specs from #{mirror}/ due to underlying error <")
      expect(err).to include("Retrying fetcher due to error (4/4): Bundler::HTTPError Could not fetch specs from #{mirror}/ due to underlying error <")
      expect(err).to include("Could not fetch specs from #{mirror}/ due to underlying error <")
    end
