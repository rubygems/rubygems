...
RSpec.describe "require 'bundler/gem_tasks'" do
  ...
  it "includes the relevant tasks" do
    with_gem_path_as(base_system_gems.to_s) do
      rake "-T", :env => { "GEM_HOME" => system_gem_path.to_s }
    end
  ...
  it "defines a working `rake install` task", :ruby_repo do
    with_gem_path_as(base_system_gems.to_s) do
      rake "install", :env => { "GEM_HOME" => system_gem_path.to_s }
    end
  ...
  it "adds 'pkg' to rake/clean's CLOBBER" do
    with_gem_path_as(base_system_gems.to_s) do
      rake %(-e 'load "Rakefile"; puts CLOBBER.inspect'), :env => { "GEM_HOME" => system_gem_path.to_s }
    end
