module Gem
  ...
  class CommandManager
    ...
    def initialize
      @commands = {}
      @common_option_list = []
      @base_command = Command.new(self, nil, nil)
      @base_command.program_name = "gem [command]"
    end
    
    def add_command(command, summary=nil)
      @commands[command.intern] = Command.new(self, command, summary)
    ...
    def add_common_option(short, long, description, &handler)
      @common_option_list << Command::Option.new(short, long, description, handler)
    end
    ...
    class Command
      ...
      def initialize(manager, command, summary=nil)
        @manager = manager
        @command = command
        @summary = summary
      ...
      def invoke(*args)
        ...
        @option_list.each do |option|
          short_names << option.short if option.short
          long_names << option.long if option.long
          parser.on(option.short, option.long, option.description) do |value|
            option.handler.call(value, options)
          end
        end
        parser.separator("")
        parser.separator("  Common Options:")
        @manager.common_option_list.each do |option|
          next if option.short && short_names.include?(option.short)
          next if option.long && long_names.include?(option.long)
          parser.on(option.short, option.long, option.description) do |value|
            option.handler.call(value, options)
          end
        end
      ...
      def add_option(short, long, description, &handler)
        @option_list << Option.new(short, long, description, handler)
      end
...
module Gem; class CommandManager
  class << self
