...
use std::ffi::{CString};
use rb_sys::{VALUE, rb_utf8_str_new, rb_define_module, rb_define_module_function};
...
pub extern "C" fn Init_custom_name() {
  ...
  let callback = unsafe { std::mem::transmute::<unsafe extern "C" fn(VALUE) -> VALUE, unsafe extern "C" fn() -> VALUE>(say_hello) };
