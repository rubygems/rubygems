...
class TestGemPlatformWheel < Gem::TestCase
  ...
  def test_platform_specificity_cross_platform_types
    ...
    # Traditional platform should be more specific for traditional user platform
    assert traditional_specificity < wheel_specificity,
      "Traditional platform should be more specific than wheel for traditional user platform"
  end
  ...
  def test_match_wheel_error_handling_with_specific
    ...
    assert_raises(ArgumentError, "Should raise error when mixing specific and keywords") do
      wheel.send(:match?, specific, ruby_abi_tag: "cr33")
    end

    # Test error when providing wrong type for specific
    assert_raises(ArgumentError, "Should raise error for wrong specific type") do
      wheel.send(:match?, "not-a-specific")
    end

    # Test error when providing neither
    assert_raises(ArgumentError, "Should raise error when no parameters provided") do
      wheel.send(:match?)
    end
  end
