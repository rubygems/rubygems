...
class TestGemSourceList < Gem::TestCase
  ...
  def test_shovel_moves_existing_to_end
    sl = Gem::SourceList.new
    ...
    # Using << on existing should behave like append and move to end
    result = (sl << uri1)
  ...
  def test_push_alias_and_append_are_equivalent
    sl = Gem::SourceList.new
    ...
    assert_equal [Gem::Source.new(uri1), Gem::Source.new(uri2)], sl.sources

    # Move uri1 to end using push
    sl.push(uri1)
    assert_equal [Gem::Source.new(uri2), Gem::Source.new(uri1)], sl.sources
  end
end
